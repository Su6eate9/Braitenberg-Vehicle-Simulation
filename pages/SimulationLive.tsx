...
const SimulationLive: React.FC = () => {
  ...
  const vehicleType =
    (location.state?.vehicleType as VehicleType) || VehicleType.LOVE;
  const simName =
    (location.state?.simName as string) || `P3: ${vehicleType.split(": ")[1]}`;
    ...
    (time: number) => {
      if (lastTimeRef.current !== 0 && isPlaying) {
        ...
        const getSensorPos = (side: "l" | "r") => {
          const baseAngle =
            vehicleType === VehicleType.FEAR ? angle + Math.PI : angle;
        ...
        switch (vehicleType) {
          ...
          case VehicleType.AGGRESSION:
            mL = baseV + sR * gain * 4.0;
            mR = baseV + sL * gain * 4.0;
            break;

          case VehicleType.LOVE:
            const loveBase = baseV * 4.0;
            const loveGain = gain * 2.0;
            mL = Math.max(0.3, loveBase - sL * loveGain);
            mR = Math.max(0.3, loveBase - sR * loveGain);
            break;

          case VehicleType.EXPLORER:
            const explorerBase = baseV * 3.5;
            const explorerGain = gain * 1.8;
            mL = Math.max(1.5, explorerBase - sR * explorerGain);
            mR = Math.max(1.5, explorerBase - sL * explorerGain);
            break;
        }
    ...
    <div className="flex flex-col h-screen bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-white overflow-hidden">
      <header className="px-6 py-4 border-b border-slate-200 dark:border-white/5 flex justify-between items-center bg-white dark:bg-surface-dark z-30 shadow-sm">
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate("/")}
            className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-white/10 text-primary"
          >
      ...
      <div
        className="h-[55vh] relative bg-slate-950 overflow-hidden cursor-crosshair touch-none border-b border-white/10"
        onMouseMove={handleLightMove}
        onTouchMove={handleLightMove}
      >
        <div
        ...
        >
          <div className="relative">
            <div className="w-10 h-12 bg-slate-800 rounded-lg border-2 border-white/20 shadow-2xl flex flex-col items-center justify-center pt-1">
              ...
              <span
                className={`material-symbols-outlined text-xl ${vDetails.color}`}
              >
                smart_toy
              </span>
            </div>
            <div className="absolute -bottom-3 left-0 right-0 flex justify-center gap-3 opacity-60">
              <div
                ...
                style={{ height: `${motorValues.l * 5}px` }}
              ></div>
              <div
                className="w-1.5 bg-amber-500 rounded-full transition-all"
                style={{ height: `${motorValues.r * 5}px` }}
              ></div>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                ...
                <span className="text-2xl font-bold font-mono text-primary">
                  {distance.toFixed(2)}
                  <span className="text-xs ml-1 font-sans">m</span>
                </span>
              </div>
              <div className="flex justify-between items-end">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-[10px] font-mono">
                    <span>M_L (PWM)</span>
                    <span>{Math.round(motorValues.l * 10)}%</span>
                  </div>
                  <div className="h-1.5 bg-slate-100 dark:bg-black/40 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-primary/50"
                      style={{ width: `${Math.min(100, motorValues.l * 15)}%` }}
                    ></div>
                  </div>
                </div>
              </div>
              <div className="space-y-4">
                <div className="space-y-2">
                  <div className="flex justify-between text-[10px] font-mono">
                    <span>S_R (Dir)</span>
                    <span>{sensorValues.r.toFixed(1)}%</span>
                  </div>
                  <div className="h-1.5 bg-slate-100 dark:bg-black/40 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-amber-500"
                      style={{ width: `${sensorValues.r}%` }}
                    ></div>
                  </div>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between text-[10px] font-mono">
                    <span>M_R (PWM)</span>
                    <span>{Math.round(motorValues.r * 10)}%</span>
                  </div>
                  <div className="h-1.5 bg-slate-100 dark:bg-black/40 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-amber-500/50"
                      style={{ width: `${Math.min(100, motorValues.r * 15)}%` }}
                    ></div>
                  </div>
                </div>
              </div>
